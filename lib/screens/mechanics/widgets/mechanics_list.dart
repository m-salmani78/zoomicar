...
class MechanicsList extends StatefulWidget {
  ...
  const MechanicsList({Key? key, required this.problem, this.filter})
      : super(key: key);

  @override
  _MechanicsListState createState() => _MechanicsListState();
}
            ...
            if (snapshot.connectionState == ConnectionState.done) {
              ...
              return FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) {
                    // print('@ Mechanic Rrror: ' + snapshot.error.toString());
                    return _buildError();
                  }
                  if (snapshot.connectionState == ConnectionState.done) {
                    final response = snapshot.data as http.Response;
                    // print('@ Mechanic StatusCode: ' +
                    //     response.statusCode.toString());
                    if (response.statusCode == 200) {
                      final response = snapshot.data as http.Response;
                      // print('@ Mechanic Response: ' + response.body);
                      ...
                      if (filter == null || filter.isEmpty) {
                        ...
                        return _buildMachanicList(
                            mechanics
                                .where((item) =>
                                    item.name.contains(widget.filter ?? ''))
                                .where((item) =>
                                    item.tags.contains(widget.problem.tag))
                                .toList(),
                            customLocation);
